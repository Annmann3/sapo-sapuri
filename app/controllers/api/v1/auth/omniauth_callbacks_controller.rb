class Api::V1::Auth::OmniauthCallbacksController < DeviseTokenAuth::OmniauthCallbacksController
  ...
  def get_redirect_route(devise_mapping)
    puts 'get_redirect_route'
    byebug
    path = "#{Devise.mappings[devise_mapping.to_sym].fullpath}/#{params[:provider]}/callback"
    klass = request.scheme == 'https' ? URI::HTTPS : URI::HTTP
    redirect_route = klass.build(host: request.host, port: request.port, path:).to_s
  end
  ...
  def omniauth_success
    ...
    if omniauth_params['provider'] == 'email'
      # provider と uidをjsonで返す
      data = {
        provider: auth_hash['provider'],
        uid: auth_hash['uid'],
        change_provider: true
      }
      redirect_to DeviseTokenAuth::Url.generate(auth_origin_url, data.as_json)
  ...
  def validate_auth_origin_url_param
    puts 'validate_auth_origin_url_param'
